# main.py — OZ 2026 УЛЬТИМА | ИСПРАВЛЕНО НАВСЕГДА | РАБОТАЕТ НА FLY.IO
import os
import json
import time
import logging
...
async def check_coin(coin: str):
    ...
    try:
        ...
        bull_div = low[-3] < low[-10] and rsi_val > rsi(data[-100:][-10:][-1]) and rsi_val < 35
        bear_div = high[-3] > high[-10] and rsi_val < rsi(data[-100:][-10:][-1]) and rsi_val > 65
...
def is_auth(request: Request):
    return request.cookies.get("auth") == "777"
...
async def root(request: Request):
    ...
    # ИСПРАВЛЕННЫЕ МАРКЕРЫ ДЛЯ ГРАФИКА
    markers = []
    ...
    <h2>ГРАФИК СИГНАЛОВ (BTCUSDT 15m)</h2>
